...
function getBranchs(callback) {
    const urlGetBranchs = `${gitInfo.domain}/${gitlabApiV}/projects/${gitInfo.projectPathEncoded}/repository/branches?private_token=${getToken()}`;
...
function checkBranch(targetBranch, branchList = []) {
    if (branchList) {
        if (!branchList.some(branch => branch.name === targetBranch)) {
...
var openMr = function (targetBranch, opt = {}) {
    try {
        ...
        getBranchs(branchs => {
            if (checkBranch(targetBranch, branchs)) {
